//https://raw.githack.com/jonesjeff/PortfolioWebsite/master/CustomJS.js embeeded link
//<script src="https://drive.google.com/uc?export=view&id=1phnXMQWkXfWBnhzXjJ-I9Y5VYVKQx45g" type="text/javascript"></script>
...
$(document).ready(function() {
  ...
  myFullpage = new fullpage('#fullpage', {
    ...
    afterLoad: function(origin, destination, direction) {
      ...
      */
      ...
      } else if (destination.index === 2 && !aboutSketchesInit) {
        aboutSketchesInit = true;
        sketch1 = new p5(eyes, 'interactiveEyesContainer');
        sketch2 = new p5(creative, 'interactiveCreativeContainer');
        sketch3 = new p5(positive, 'interactivePositiveContainer');
  ...
  barba.init({
    ...
    transitions: [{
      ...
      leave(data) {
        gsap.to(data.current.container, 0.25, {
          opacity: 0, onComplete: this.async(),
        });
      },
      enter(data) {
        data.current.container.parentNode.removeChild(data.current.container);
        gsap.from(data.next.container, 0.25, {
          opacity: 0, onComplete: this.async(),
        });
      },
      after(data) {
        console.log("update: " + data.next.namespace);
...
let eyes = function(s1) {
  s1.setup = function() {
    console.log("Creating new canvas");
    let cnv = s1.createCanvas(200, 200);
    cnv.style('display', 'block');
    s1.pixelDensity(1);
  }
  
  s1.draw = function() {
    s1.rect(0, 0, s1.width, s1.height, 20);
  }
}

let creative = function(s2) {
  s2.setup = function() {
    console.log("Creating new canvas");
    let cnv = s2.createCanvas(200, 200);
    cnv.style('display', 'block');
    s2.pixelDensity(1);
  }
  
  s2.draw = function() {
    s2.rect(0, 0, s2.width, s2.height, 20);
  }
}

let positive = function(s3) {
  s3.setup = function() {
    console.log("Creating new canvas");
    let cnv = s3.createCanvas(200, 200);
    cnv.style('display', 'block');
    s3.pixelDensity(1);
  }
  
  s3.draw = function() {
    s3.rect(0, 0, s3.width, s3.height, 20);
  }
}
